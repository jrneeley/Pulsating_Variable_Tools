...
# robust median statistic
    ...
    # first filter out nan mags
    filters = filters[~np.isnan(mags)]
...
# each individual variability index (0=not variable 1=variable)
    ...
    if os.path.exists('var_search/vi.txt'):
        ...
        for i in range(nstars):
            ...
            for j in range(n_filters):
                ...
                num_actual_obs = data['n_frames'][i][j]
            ...
            if fail == 1:
                ...
                # If we are removing flagged frames, do that here
                    ...
                    keep = (~np.isnan(mag_all)) & (data['frame flags'][i] == 0)

                # calculate various variability indices for this star
                vi[i] = compute_variability_index(data['filter names'][i][keep],
                    data['mjds'][i][keep], mag_all[keep],
                    data['emags'][i][keep], max_time=0.02)
    ...
    clipped, bins, std = binned_sigma_clip(data['mean_mag'][:,1], vi_copy,
...
#### GOOD TO HERE, NOW NEED TO WORRY ABOUT CHIP
